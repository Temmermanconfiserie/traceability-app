...
# --- TIJDELIJKE WACHTWOORD RESET ROUTE ---
@app.route("/reset-admin-password-now/f7g8h9j0k1l2")
def reset_admin_password():
    # Hier stellen we de admin gebruiker en het nieuwe wachtwoord in
    admin_username_to_reset = "Temmermanconfiserie"
    new_password = "NewPassword123" # Gebruik dit om in te loggen!
    ...
    try:
        with conn:
            with conn.cursor() as cur:
                ...
                # Maak de admin gebruiker aan als deze niet bestaat, of update het wachtwoord als hij wel bestaat
                cur.execute(
                    """
                    INSERT INTO users (username, password, role) VALUES (%s, %s, 'admin')
                    ON CONFLICT (username) DO UPDATE SET password = EXCLUDED.password;
                    """,
                    (admin_username_to_reset, hashed_password)
                )
        return f"Wachtwoord voor '{admin_username_to_reset}' is ingesteld. Je kunt nu inloggen met het wachtwoord: {new_password}"
    except Exception as e:
        return f"Fout bij het instellen van het wachtwoord: {e}"
    finally:
        if conn is not None:
            conn.close()
...
# --- API Routes ---
# ... (alle andere API routes blijven ongewijzigd) ...
