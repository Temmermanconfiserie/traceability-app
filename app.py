...
def delete_user(user_id):
    if user_id == current_user.id:
        return jsonify(status="error", message="Je kunt jezelf niet verwijderen."), 403
    conn = get_db_connection()
    try:
        with conn:
            with conn.cursor() as cur:
                cur.execute("DELETE FROM users WHERE id = %s;", (user_id,))
        return jsonify(status="success", message="Gebruiker succesvol verwijderd.")
    except Exception as e:
        conn.rollback()
        return jsonify(status="error", message=str(e)), 500
    finally:
...
@app.route("/api/leveranciers/<int:leverancier_id>", methods=['DELETE'])
@login_required
@admin_required
def delete_leverancier(leverancier_id):
    conn = get_db_connection()
    try:
        with conn:
            with conn.cursor() as cur:
                cur.execute("SELECT 1 FROM Voorraad_Inkomend WHERE leverancier_id = %s LIMIT 1;", (leverancier_id,))
                if cur.fetchone():
                    raise Exception("Deze leverancier kan niet worden verwijderd omdat er nog voorraad aan gekoppeld is.")
                cur.execute("DELETE FROM Leveranciers WHERE id = %s;", (leverancier_id,))
        return jsonify(status="success", message="Leverancier succesvol verwijderd.")
    except Exception as e:
        conn.rollback()
        return jsonify(status="error", message=str(e)), 500
    finally:
...
@app.route("/api/klanten/<int:klant_id>", methods=['DELETE'])
@login_required
@admin_required
def delete_klant(klant_id):
    conn = get_db_connection()
    try:
        with conn:
            with conn.cursor() as cur:
                cur.execute("SELECT 1 FROM Verzendingen WHERE klant_id = %s LIMIT 1;", (klant_id,))
                if cur.fetchone():
                    raise Exception("Deze klant kan niet worden verwijderd omdat er nog verzendingen aan gekoppeld zijn.")
                cur.execute("DELETE FROM Klanten WHERE id = %s;", (klant_id,))
        return jsonify(status="success", message="Klant succesvol verwijderd.")
    except Exception as e:
        conn.rollback()
        return jsonify(status="error", message=str(e)), 500
    finally:
...
@app.route("/api/producten/inkomend/<string:referentie>", methods=['DELETE'])
@login_required
@admin_required
def delete_inkomend_product(referentie):
    conn = get_db_connection()
    try:
        with conn:
            with conn.cursor(cursor_factory=RealDictCursor) as cur:
                cur.execute("SELECT id FROM Inkomende_Producten WHERE referentie = %s;", (referentie,))
                product = cur.fetchone()
                if not product: raise Exception("Product niet gevonden.")
                product_id = product['id']

                cur.execute("SELECT 1 FROM Voorraad_Inkomend WHERE inkomend_product_id = %s LIMIT 1;", (product_id,))
                if cur.fetchone(): raise Exception("Product is in gebruik in de voorraad.")
                cur.execute("SELECT 1 FROM Recept_Componenten WHERE inkomend_product_id = %s LIMIT 1;", (product_id,))
                if cur.fetchone(): raise Exception("Product is in gebruik in een recept.")
                cur.execute("SELECT 1 FROM Uitgaande_Producten WHERE bron_inkomend_id = %s LIMIT 1;", (product_id,))
                if cur.fetchone(): raise Exception("Product is direct gekoppeld aan een uitgaand product.")

                cur.execute("DELETE FROM Inkomende_Producten WHERE id = %s;", (product_id,))
        return jsonify(status="success", message="Product succesvol verwijderd.")
    except Exception as e:
        conn.rollback()
        return jsonify(status="error", message=str(e)), 500
    finally:
...
def export_inkomende_voorraad():
    ...
    cur = conn.cursor(cursor_factory=RealDictCursor)
    cur.execute("""
        SELECT ip.referentie, ip.productnaam, SUM(vi.resterend_gewicht_kg) as totaal_resterend_kg
        FROM Voorraad_Inkomend vi
        JOIN Inkomende_Producten ip ON vi.inkomend_product_id = ip.id
        GROUP BY ip.referentie, ip.productnaam
...
def export_afgewerkte_voorraad():
    ...
    cur = conn.cursor(cursor_factory=RealDictCursor)
    cur.execute("""
        SELECT up.referentie, up.productnaam, SUM(pb.resterend_aantal) as totaal_resterend_stuks
        FROM Productie_Batch pb
        JOIN Uitgaande_Producten up ON pb.uitgaand_product_id = up.id
        GROUP BY up.referentie, up.productnaam
