        ...
        with conn:
            with conn.cursor() as cur:
                ...
                cur.execute(
                    ...
                    (hashed_password, admin_username_to_reset)
                )
        return f"Wachtwoord voor '{admin_username_to_reset}' is gereset. Je kunt nu inloggen met het wachtwoord: {new_password}"
    except Exception as e:
        return f"Fout bij het resetten van het wachtwoord: {e}"
    finally:
        if conn is not None:
            conn.close()
...
# --- De rest van de API routes en helper functies blijven ongewijzigd ---
